...
fun addNewStudent(fname: String, lname: String, pwd: String, email: String, classroom_id: String) : MutableMap<String, Any>? {
    ...
    // Add student to classroom (convoluted)
    val newList = mutableListOf<String>()
    val classroom = getClassroom(classroom_id)
    val assignments = classroom?.get("students").toString()
        .replace("[", "")
        .replace("]","")
    // Add id to assignments (convoluted)
    val entries = assignments.split(", ")
    newList.addAll(entries)
    newList.add(id)
    classroom?.set("students", newList)
...
fun getTeacherTemplate() : MutableMap<String, Any>? {
    return getDocumentFromDB(Constants.teachers_col, Constants.teacher_form, Constants.db)
}
...
fun getTeacher(id: String) : MutableMap<String, Any>? {
    return getDocumentFromDB(Constants.teachers_col, id, Constants.db)
}

// Classroom methods
fun getClassroomTemplate() : MutableMap<String, Any>? {
    return getDocumentFromDB(Constants.classrooms_col, Constants.classroom_form, Constants.db)
}

// All classrooms must have a teacher associated with them
fun addNewClassroom(teacher_email : String, pwd: String) : MutableMap<String, Any>? {
    // First, make sure that the teacher actually exists.
    val teacher_id = createID(teacher_email, pwd)
...
fun getClassroom(classroom_id: String) : MutableMap<String, Any>? {
    return getDocumentFromDB(Constants.classrooms_col, classroom_id, Constants.db)
}

// Assignments methods
fun getAssignmentTemplate() : MutableMap<String, Any>? {
    return getDocumentFromDB(Constants.assignments_col, Constants.assignment_form, Constants.db)
}
...
fun addNewAssignment(classroom_id: String, title : String, desc : String, problem : String,
    ...
    val newList = mutableListOf<String>()
    val classroom = getClassroom(classroom_id)
    val assignments = classroom?.get("assignments").toString()
        .replace("[", "")
        .replace("]","")
    // Add id to assignments (convoluted)
    val entries = assignments.split(", ")
    newList.addAll(entries)
    newList.add(id)
    classroom?.set("assignments", newList)
    println(classroom)
    val class_ref = Constants.db.collection(Constants.classrooms_col).document(classroom_id)
    class_ref.set(classroom!!)
